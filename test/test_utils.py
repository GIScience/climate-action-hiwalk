...
def id_filter() -> str:
    ...
    # way/156202491 is part of probable_no
...
def test_construct_filter_verify_probable_no():
    filters = construct_filters()
    probable_filter = filters[Rating.PROBABLE_NO]
    verify(probable_filter)


def test_construct_filter_validate_probable_no(request_ohsome, id_filter):
    filters = construct_filters()
    ohsome_filter = filters[Rating.PROBABLE_NO]
    ohsome_filter = f'({ohsome_filter}) and ({id_filter})'
    elements = request_ohsome(filter=ohsome_filter).as_dataframe(multi_index=False)
...
def test_construct_filter_verify_inaccessible():
    filters = construct_filters()
    probable_filter = filters[Rating.INACCESSIBLE]
    verify(probable_filter)


def test_construct_filter_validate_inaccessible(request_ohsome, id_filter):
    filters = construct_filters()
    ohsome_filter = filters[Rating.INACCESSIBLE]
    ohsome_filter = f'({ohsome_filter}) and ({id_filter})'
...
def test_get_color():
    ...
    expected_input = pd.Series([Rating.EXCLUSIVE, Rating.PROBABLE_YES, Rating.INACCESSIBLE])
